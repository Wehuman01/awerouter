...
You may run these read-only or non-interactive commands: `awerouter init`, `awerouter config path`, `awerouter config show`, `awerouter config validate`, `awerouter config edit`, `awerouter list`, `awerouter serve status` (read-only instance listing), `awerouter usage stats`, `awerouter usage log`, `awerouter usage calibrate`, `awerouter usage savings`.
